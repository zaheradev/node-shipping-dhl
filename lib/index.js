...
function DHL (options) {
  ...
  self.url = {
  	staging: 'https://xmlpitest-ea.dhl.com/XMLShippingServlet',
  ...
  function generateMessageReference () {
    ...
    numberOfDigits = randomInt(22, 27);
    randomDigits = ['Z','A','H','E','R','A'];
   ...
   function postToDHL2 (body, callback) {
	...
	const options = {
		url: self.url[self.options.mode],
		method: 'POST',
		headers: {
	...
	request(options, {formData: body}, function (err, res, body) {
		if(err){
			callback(err, null);
		}
...
function splitter(str, l){
  ...
  function buildShipXML (data) {
    ...
    xml = concatKeyValuePair(xml, 'RegionCode', 'AP');
    ...
    if (!Array.isArray(data.receiver.address)) {
       data.receiver.address = data.receiver.address.join(", ");
    }
      
    splitter(data.receiver.address, 44).forEach(function (line) {
        // Don't append empty strings.
        if (line !== '') {
        consignee = concatKeyValuePair(consignee, 'AddressLine', line);
        }
    });
    data.receiver.address = data.receiver.address.slice(0, 3)
    ...
    xml = concatKeyValuePair(xml, 'Dutiable', {
      DeclaredValue: Math.max(0.01, data.commodities.reduce(function (acc, curr) {
    ...
    exportDeclaration = concatKeyValuePair(exportDeclaration, 'ExportReasonCode',  'P');
	  // exportDeclaration = concatKeyValuePair(exportDeclaration, 'InvoiceNumber',  data.invoiceNumber);
	  // exportDeclaration = concatKeyValuePair(exportDeclaration, 'InvoiceDate',  data.invoiceDate);
    ...
    xml = concatKeyValuePair(xml, 'ShipmentDetails', {
      ...
      Pieces: data.parcels.map(function (parcel, index) {
        return {
          Piece: {
            PieceID:     index + 1,
            PackageType: 'YP', // EE (express envelope) or OD (other DHL)
            Weight:      parcel.weight,
            // http://www.dhl.com/en/tools/volumetric_weight_express.html
  ...
  function ship (data, xmlCallback, callback) {
    if (!callback) {
      throw new Error("no callback specified");
    }
